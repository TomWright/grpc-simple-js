...
`))
		...
		if err := p.generateMappersImports(f, out, outputPath, grpcWebPackage); err != nil {
...
func (p *Runner) generateMappersImports(f *protogen.File, out *protogen.GeneratedFile, currentPath string, grpcWebPackage string) error {
	...
	addRequired := func(descriptor protoreflect.Descriptor) {
		...
		rMapper := &requiredImport{
			FileDesc:     requiredFile,
			ImportName:   mapping.PkgToImportPkg(mapping.DescriptorPackage(requiredFile)) + "_mapper",
		...
		required = append(required, r, rMapper)
	}
